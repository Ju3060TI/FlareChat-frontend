...
// Zentrale Nachrichten-Logik: Senden via WebSocket, Fallback via HTTP

import { apiFetch } from '../api/client.js';

// ============================================================
// 1. NACHRICHT SENDEN (via WebSocket + Fallback HTTP)
// ============================================================
...
export async function sendMessage(text, target, type, state) {
  ...
  // Versuche zuerst über WebSocket zu senden
  const ws = window.ws; // wird in main.js gesetzt
  if (ws && ws.readyState === WebSocket.OPEN) {
    try {
      ...
      console.warn('WebSocket-Send fehlgeschlagen, fallback auf HTTP', e);
    }
  }

  // Fallback: HTTP-API (nur wenn WebSocket nicht verfügbar)
  console.log('📤 Nachricht via HTTP gesendet (Fallback)');
  if (type === 'friend') {
